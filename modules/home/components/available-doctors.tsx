...
export const AvailableDoctors = () => {
  const renderItem = ({ item }: { item: typeof doctor }) => {
    return (
      <View style={tw.style('h-[130px] bg-white p-3 pb-0 rounded-xl', { width: ITEM_WIDTH })}>
        {/* top section */}
        <View style={tw`flex-row justify-between border-b border-[#adadad] pb-2`}>
          <View style={tw`w-[76px] aspect-square rounded-xl overflow-hidden`}>
            <Image
              source={require('~/assets/images/dr-priscilla.jpeg')}
              contentFit="cover"
              transition={300}
          ...
          <View style={tw`flex-1 mx-1.5 gap-1`}>
            ...
            <Text style={tw`text-primary text-xs font-normal`} numberOfLines={1}>
              {item.role} -{' '}
              <Text style={tw`text-neutral-600 text-xs font-normal`}>{item.hospital}</Text>
            </Text>
            <Text style={tw`text-[#248A3D] text-xs font-normal`}>{item.status}</Text>
        ...
        <View style={tw`flex-row justify-between items-center flex-1`}>
          ...
          <Text style={tw`text-xs font-medium`}>
            {formatNaira(item.rates.minimum)} - {formatNaira(item.rates.maximum)} / hr
          </Text>
        </View>
      </View>
    );
  ...
  return (
    <View style={tw`gap-3 px-5`}>
      <View style={tw`flex-row justify-between`}>
        <Text style={tw`text-xs font-semibold leading-none`}>Doctors available </Text>
